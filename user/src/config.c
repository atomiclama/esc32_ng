...
const char *configParameterStrings[] = { "CONFIG_VERSION", "STARTUP_MODE", "BAUD_RATE", "PTERM", "ITERM", "FF1TERM",
        "FF2TERM", "CL1TERM", "CL2TERM", "CL3TERM", "CL4TERM", "CL5TERM", "SHUNT_RESISTANCE", "MIN_PERIOD",
        "MAX_PERIOD", "BLANKING_MICROS", "ADVANCE", "START_VOLTAGE", "GOOD_DETECTS_START", "BAD_DETECTS_DISARM",
        "MAX_CURRENT", "SWITCH_FREQ", "MOTOR_POLES", "PWM_MIN_PERIOD", "PWM_MAX_PERIOD", "PWM_MIN_VALUE",
        "PWM_LO_VALUE", "PWM_HI_VALUE", "PWM_MAX_VALUE", "PWM_MIN_START", "PWM_RPM_SCALE", "FET_BRAKING", "PNFAC",
        "INFAC", "THR1TERM", "THR2TERM", "START_ALIGN_TIME", "START_ALIGN_VOLTAGE", "START_STEPS_NUM",
        "START_STEPS_PERIOD", "START_STEPS_ACCEL", "PWM_LOWPASS", "RPM_MEAS_LP", "SERVO_DUTY", "SERVO_P", "SERVO_D",
        "SERVO_MAX_RATE", "SERVO_SCALE", "ESC_ID", "DIRECTION" };

const char *configFormatStrings[] = { "%f",	    // CONFIG_VERSION
        "%.0f",	    // STARTUP_MODE
        "%.0f baud",    // BAUD_RATE
        "%.3f",	    // PTERM
        "%.5f",	    // ITERM
...
int configWriteFlash(void) {
    ...
    if ((FLASHStatus = FLASH_ErasePage(FLASH_WRITE_ADDR)) == FLASH_COMPLETE) {
        ...
        while (FLASHStatus == FLASH_COMPLETE && address < sizeof(p)) {
            if ((FLASHStatus = FLASH_ProgramWord(FLASH_WRITE_ADDR + address, *(uint32_t *) ((char *) p + address)))
                    != FLASH_COMPLETE)
                break;
