...
void DMA1_Channel1_IRQHandler(void) {
    ...
    if (!fetCommutationMicros
            || ((currentMicros >= fetCommutationMicros) ?
                    (currentMicros - fetCommutationMicros) : (TIMER_MASK - fetCommutationMicros + currentMicros))
                    > adcblankingMicros) {
#ifdef ADC_FAST_SAMPLE
        ...
        if ((avgA + avgB + avgC) / histSize > (ADC_MIN_COMP * 3) && state != ESC_STATE_DISARMED) {
            ...
            periodMicros =
                    (currentMicros >= detectedCrossing) ?
                            (currentMicros - detectedCrossing) : (TIMER_MASK - detectedCrossing + currentMicros);
...
//		    crossingPeriod = (crossingPeriod*15 + periodMicros)/16;
                    ...
                    timerSetAlarm1(
                            crossingPeriod / 2 - (ADC_DETECTION_TIME * (histSize + 2)) / 2 - ADC_COMMUTATION_ADVANCE,
                            fetCommutate, crossingPeriod);
