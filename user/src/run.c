...
void runSetpoint(uint16_t val) {
    ...
    if (state == ESC_STATE_RUNNING) {
        if (runMode == OPEN_LOOP) {
            ...
            target = ((sqrtf(p[THR1TERM] * p[THR1TERM] + 4.0f * p[THR2TERM] * target) - p[THR1TERM])
                    / (2.0f * p[THR2TERM]));
    ...
    if (state == ESC_STATE_RUNNING && setpoint != lastPwm) {
        ...
        // THRUST Mode
            ...
            if (targetThrust > 0.0f) {
                ...
                target = ((sqrtf(p[THR1TERM] * p[THR1TERM] + 4.0f * p[THR2TERM] * targetThrust) - p[THR1TERM])
                        / (2.0f * p[THR2TERM]));
            }
...
static inline void runThrotLim(int32_t duty) {
    ...
    if (p[MAX_CURRENT] > 0.0f) {
        ...
        if (p[CL1TERM] != 0.0f) {
            maxVolts = p[CL1TERM] + p[CL2TERM] * rpm + p[CL3TERM] * p[MAX_CURRENT] + p[CL4TERM] * rpm * maxCurrentSQRT
                    + p[CL5TERM] * maxCurrentSQRT;
